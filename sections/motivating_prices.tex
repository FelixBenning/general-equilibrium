...
\begin{lemma}[Income in Variable \& Fixed Cost Case]
	...
	\[
		...
		= l_i^{(j)}\Bigl[1+\frac{1}{L_i/f_i^{(j)}-1}\Bigr]
	\]
\end{lemma}
\begin{proof}
	...
	\]
	The average labour cost of specialization \(j\) is moreover given by
	\[
		\bar{l}_i^{(j)}
		= \frac{L_i}{x^{(j)}}
	...
	\[
		\mu(p, X_i)
		= L_i \max_{j=1,\dots,\dims} p_j \frac{x^{(j)}}{L_i}
		= L_i \max_{j=1,\dots,\dims} \frac{p_j}{\bar{l}_i^{(j)}}.
		\qedhere
	\]
\end{proof}
...
What is interesting to note here, is that (hourly) wages increase with total
labour time \(L_i\), as average cost decrease with \(L_i\). The size of this
effect increases with the size of the fixed costs. If the fixed costs are close
to \(L_i\) small decreases of \(L_i\) might make producing \(j\) completely
unviable, but at the very least cut heavily into the little time left for
actually producing \(j\). Or in other words average costs have singularities at
\(f_i^{(j)}\)
\[
	\lim_{L_i\downarrow f_i^{(j)}} \bar{l}_i^{(j)}
	= \lim_{L_i\downarrow f_i^{(j)}}l_i^{(j)}\Bigl[1+\frac{1}{L_i/f_i^{(j)}-1}\Bigr]
	= \infty
\]
