...
\begin{align*}
	\tfrac1n D^{(j)}(p)
	&= \frac1n\sum_{i=1}^n(
		1- \ind_{Z_i^{(j)}\|w_i^{(-j)}\|_\infty \le p_j}
		\ind_{Z_i^{(j)}\le l_i^{(j)}}
	)\\
	&\to\begin{aligned}[t]
		&= 1- \Pr(
...
willingness to buy for product \(j\). If the probability that people are
willing to do it themselves (DIY) is small, which is likely for a large number
of products, then due to
\[
	0
