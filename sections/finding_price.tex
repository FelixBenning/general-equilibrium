...
\begin{lemma}[Production Frontier]
	If \(X\subset \real_{\ge 0}^\dims\) is a compact, convex \ref{eq: lower
	layer}, then for any \(x\in \partial X\), there exists
	\(p\in\real_{\ge 0}^\dims\) such that
	\begin{equation}
	...
	\]
	If additionally \(x\in\real_{>0}^\dims\) then \(p\in\real_{\ge 0}^\dims\) is
	necessary.
\end{lemma}
\begin{proof}
	...
	there exists
	\(p\in\real^\dims\) and \(c\in\real\) with
	\[
	...
	Before we show, that there exists a \(p\) with non-negative entries, let us
	consider the strict case
	\(x=(x^{(1)},\dots,x^{(\dims)})\in\real_{>0}^\dims\).
	...
	Going back to the general case, if we allow entries of \(x\) to be zero, the
	same technique still applies to the entries where \(x^{(i)}>0\). So let us
	now consider the remaining entries, where \(x^{(i)}=0\). If we have \(p_i<0\)
	here, we can replace \(p\) by \(\tilde{p}\) with the \(i\)-th entry replaced
	by \(0\). We need to show, that \eqref{eq: x is max w.r.t. p} still holds for
	\(\tilde{p}\). So let there be \(y\in X\) breaking \eqref{eq: x is max w.r.t.
	p} for \(\tilde{p}\). As \(\tilde{p}_i=0\) we can w.l.o.g. assume that
	this breaking \(y\) satisfies \(y^{(i)}=0\). But this implies
	\[
		\langle y, p\rangle
		\overset{y_i=0}= \langle y, \tilde{p}\rangle
		> \langle x, \tilde{p}\rangle
		\overset{x_i=0}= \langle x, p\rangle,
	\]
	which is a contradiction, because \eqref{eq: x is max w.r.t. p} holds for 
	the original \(p\).
\end{proof}
