...
\begin{lemma}[Production Frontier]
	If \(X\) is a compact, convex \ref{eq: lower layer}, then for any \(x\in
	\partial X\), there exists
	...
	necessary. If we only require \(x\in\real_{\ge 0}^\dims\), then there exists
	a \(p\in\real_{\ge 0}^\dims\) satisfying \eqref{eq: prod frontier p}, but
	other \(p\) might satisfy \eqref{eq: prod frontier p} with negative entries
	where \(x\) has zero entries.
\end{lemma}
\begin{proof}
