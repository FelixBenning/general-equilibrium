...
The total production capabilities of a society is given by the Minkowski sum of
...
\begin{lemma}[Production Frontier]
	If \(X\) is a closed, convex \ref{eq: lower layer}, then for any \(x\in
	\partial X \cap \real_{>0}^\dims\) there exists \(p\in\real_{\ge 0}^\dims\)
	such that
	\[
		x \in \argmax_{x\in X} \langle p, x\rangle = \nabla_p \mu(p, X)
	\]
	and therefore
	\[
